...
//            ch = *p;
            ...
            if ((ch < 'A' || ch > 'Z') && ch != '_') {
                return NX_HTTP_PARSE_INVALID_METHOD;
            }
        ...
        case sw_method:
            if (ch == ' ') {//after mothod
                ...
                case 3:
                    if (nx_str3_cmp(m, 'G', 'E', 'T', ' ')) {
                        r->method = NX_HTTP_GET;
                        break;
                    }
                ...
                case 4:
                    if (nx_str3Ocmp(m, 'P', 'O', 'S', 'T')) {
                        r->method = NX_HTTP_POST;
                        break;
                    }

                    if (nx_str4cmp(m, 'H', 'E', 'A', 'D')) {
                        r->method = NX_HTTP_HEAD;
                        break;
                    }

                    break;
                default:
                    r->method = NX_HTTP_UNKNOWN;
                    break;
                }
                state = sw_spaces_before_uri;//found valid method
                break;
            }
        ...
        case sw_spaces_before_uri:
            ...
            switch (ch) {
                case ' ':
                    break;
                default:
                    return NX_HTTP_PARSE_INVALID_REQUEST;
            }
            break;
    ...
    return NX_AGAIN;//should add the fd to timer
    ...
    for (pi = r->pos; pi < r->last; pi++) {
        ...
        case sw_key:
            if (ch == ' ') {
                ...
                state = sw_spaces_before_colon;//key : value
                break;
            }

            if (ch == ':') {
                r->cur_header_key_end = p;
                state = sw_spaces_after_colon;//key:value
                break;
            }
        ...
        case sw_spaces_before_colon:
            if (ch == ' ') {
                ...
                return NX_HTTP_PARSE_INVALID_HEADER;
            }
        ...
        case sw_value:
            if (ch == CR) {// \r
                r->cur_header_value_end = p;//full finished
                state = sw_cr;
            }

            if (ch == LF) {// \n
                r->cur_header_value_end = p;//finish one key:value
                state = sw_crlf;
            }
        ...
        case sw_cr:
            if (ch == LF) { // \r\n
                ...
                hd = (nx_http_header_t *)malloc(sizeof(nx_http_header_t));
